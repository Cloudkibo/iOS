            ...
            do{ var chatsData=try Data.init(contentsOf: ubiquityURL!)
                ...
                for fileRows in FilesDataJSONobject
                {
                   ...
                   copyToAppContainer(filename: files["file_name"].string!)
                    ...
                    sqliteDB.saveFile(files["to"].string!, from1: files["from"].string!, owneruser1: username!, file_name1: files["file_name"].string!, date1: nil, uniqueid1: files["uniqueid"].string!, file_size1: files["file_size"].string!, file_type1: files["file_type"].string!, file_path1: files["file_path"].string!, type1: files["type"].string!, caption1: files["file_caption"].string!)
                }
            }
            catch{
                print("error reading \(filename.removeCharsFromEnd(5)) table from icloud")
